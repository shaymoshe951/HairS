...
def create_drawing_interface():
    ...
    def process_drawing(image_data):
        try:
            ...
            drawn_pixels = []
            background_color = np.array([173, 216, 230], dtype=np.float32)  # Light blue RGB
            tolerance = 30.0  # Color tolerance
            ...
            for y in range(0, height, step):
                for x in range(0, width, step):
                    if y < height and x < width:
                        pixel = img_array[y, x].astype(np.float32)  # Convert to float to avoid overflow

                        # Check if pixel is significantly different from background
                        color_diff = np.sum(np.abs(pixel - background_color))
                        if color_diff > tolerance:
                            drawn_pixels.append({
                                "x": int(x),
                                "y": int(y),
                                "color": pixel.astype(np.uint8).tolist()  # Convert back to uint8 for display
                            })
